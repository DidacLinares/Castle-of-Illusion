...
void Player::update(int deltaTime) {
	...
	if(Game::instance().getKey(GLFW_KEY_LEFT)) {
		...
		if (map->collisionMoveLeft(posPlayer, glm::ivec2(HITBOX_X, HITBOX_Y))) {
			posPlayer.x += 2;
		...
		if (map->collisionMoveRight(posPlayer, glm::ivec2(HITBOX_X, HITBOX_Y))) {
			posPlayer.x -= 2;
	...
	if (bJumping) {
		...
		if (jumpAngle == 180) {
			...
			if (jumpAngle > 90) bJumping = !map->collisionMoveDown(posPlayer, glm::ivec2(HITBOX_X, HITBOX_Y), &posPlayer.y);
		}
	}
	else {
		posPlayer.y += FALL_STEP;
		if (map->collisionMoveDown(posPlayer, glm::ivec2(HITBOX_X, HITBOX_Y), &posPlayer.y)) {
